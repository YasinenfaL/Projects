...
for col in cat_cols:
    eda.cat_summary(df, col, plot=False)

    # for col in num_cols:
    eda.target_summary_with_num(df, "booking_status", col)

    # for col in cat_cols:
    eda.target_summary_with_cat(df, "booking_status", col)
...
df = eda.one_hot_encoder(df, ohe_cols)
